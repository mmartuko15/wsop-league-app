...
with tabs[2]:
    ...
    if new_log is not None:
        try:
            ...
            # Update Players roster (auto-add)
            ...
            # Update Pools_Ledger WITHOUT nonlocal: build a new DF and assign
            ...
            pools = pd.concat([pools, pd.DataFrame(rows, columns=pools.columns)], ignore_index=True)
...
with tabs[5]:
    ...
    amount = st.number_input("Amount", value=200.00, step=50.0)
    date = st.date_input("Date")
    method = st.text_input("Method (cash, Zelle, etc.)", value="cash")
    note = st.text_input("Note", value="")
    if st.button("Add Buy-In Entry"):
        s = sheet_map.get("Series_BuyIns", pd.DataFrame(columns=["Player","Amount","Date","Method","Note"])).copy()
        s.loc[len(s)] = [sel_player, float(amount), str(date), method, note]
        sheet_map["Series_BuyIns"] = s
        st.success("Buy-in recorded.")

with tabs[6]:
    st.subheader("Player Finances")
...
with tabs[9]:
    ...
    if st.button("Publish tracker.xlsx to GitHub"):
        if not owner_repo or not branch or not gh_token:
            st.error("Provide repo, branch, and a GitHub token (repo scope).")
        else:

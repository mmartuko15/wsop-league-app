...
import streamlit as st, pandas as pd, requests, base64, re
...
def get_latest_commit_and_file(owner_repo, branch, token):
    headers = {"Authorization": f"token {token}"} if token else {}
    # Latest commit that touched tracker.xlsx
    cr = requests.get(f"https://api.github.com/repos/{owner_repo}/commits",
                      headers=headers, params={"path":"tracker.xlsx","sha":branch}, timeout=20)
    latest = cr.json()[0] if cr.status_code==200 and len(cr.json())>0 else None
    sha = latest.get("sha","") if latest else ""
    # Associated PRs (preview API)
    prn = ""
    if sha:
        prr = requests.get(f"https://api.github.com/repos/{owner_repo}/commits/{sha}/pulls",
                           headers={**headers, "Accept":"application/vnd.github.groot-preview+json"}, timeout=20)
        if prr.status_code==200 and len(prr.json())>0:
            pr = prr.json()[0]
            if pr.get("merged_at"):
                prn = f"#{pr.get('number')}"
    return sha, prn
...
if mode == "Upload file":
    ...
    if uploaded:
        b = uploaded.read()
        sheet_map = read_tracker_bytes(b)
    ...
    if st.sidebar.button("Fetch via API now"):
        try:
            ...
            commit_sha, merged_pr = get_latest_commit_and_file(owner_repo, branch, token)
...
if merged_pr: extra.append(f"Merged PR: {merged_pr}")
st.info("**Data source:** " + source_label + ((" • " + " • ".join(extra)) if extra else ""))
...
with tabs[4]:
    ...
    if display_val:
        # try to format numeric overrides
        try:
            amt = float(display_val.replace("$","").replace(",",""))
            amt_str = f"${amt:,.2f}"
        except:
            amt_str = display_val
    else:
        amt_str = f"${highhand_total:,.2f}"
    st.write(f"**Current Holder:** {holder if holder else '—'}")
    st.write(f"**Hand:** {hand_desc if hand_desc else '—'}")
    st.write(f"**Jackpot Value:** {amt_str}")
